...
async function run() {
  try {
    ...
    const taskCollection = client.db('wishTask').collection('allTasks');

    app.get('/allTasksData' ,async (req,res)=>{
    ...
    app.post('/allTasksData',async(req,res)=>{
        const task = req.body;
        const result = await taskCollection.insertOne(task);
        res.send(result);
    })

    app.put('/allTasksData/:id',async(req,res)=>{
        const id = req.params.id;
    ...
    app.delete('/allTasksData/:id', async(req,res)=>{
        const id = req.params.id;
        const query = {_id: new ObjectId(id)}
